...
struct VaultEntry {
    ...
    VaultEntry() : id(-1), createdAt(0), lastModified(0), lastAccessed(0), passwordExpiry(0), totp_secret("") {}
    VaultEntry(int id, std::string t, std::string u, std::string n) 
        : id(id), title(std::move(t)), username(std::move(u)), notes(std::move(n)), 
          createdAt(0), lastModified(0), lastAccessed(0), passwordExpiry(0), totp_secret("") {}
};
